...
int tputc(tputc_t ch);   // fast putchar()
static inline void gotoxy(int col, int row) { tputs(tgoto(CM, col, row), 1, tputc); }
...
typedef struct termOptStr       // saved terminal settings
{
    TERMIO      o_termio;
    int         flags;
} termOptStr;
