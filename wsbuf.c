// ****************************************************************************
// wsbuf.c  Macro Screen Editor buffer routines
//
...
#endif
    clearScreen();
    for (int j = 0; j < SCRMAXHT; j++)
        for (int i = 0; i < SCRMAXWD; i++)
            screenImage[j][i] = ' ';
}
...
void insert(char* p, const char* str, int n)
{
    if (buffer[b].readOnly)
        throw_error("%s is a read-only file", buffer[b].fname);
...
bool insertFile(const char* fileName, enum InsertMode mode)
{
    ...
    if (fp == NULL && mode == READ)
        throw_error("can't find file '%s'", fileName);
    ...
    else
    {
        ...
        if (!(fseek(fp, (long)0, 2) == 0 && (size = ftell(fp)) != EOF
            && fseek(fp, (long)0, 0) == 0))
            throw_error("can't position file '%s'", fileName);
...
void writeToFile(const char* fName, const char* fPath, char* start, char* end)
{
    ...
    fp = fopen(fPath, "w");
    if (!fp)
        throw_error("can't write file '%s'", fName);
