...
void Error_init_valist(Error* e, const char* fmt, va_list ap) {
    ...
    e->message = (char*)malloc(max_messageLen);
    if (e->message) {
        vsnprintf(e->message, max_messageLen, fmt, ap);
    }
}
...
void throw_error(const char* fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    Error_init_valist(&current_error, fmt, ap);
    va_end(ap);
    longjmp(exception_env, EX_ERROR);
}

void throw_cancel() {
    longjmp(exception_env, EX_CANCEL);
}
...
void saveIfOpen()
{
    if (buffer[b].changed)
    {
        ...
        else if (key == 'Y')
        {
            ...
            {
                initCmdBuf();
                getCommand("Save buffer as:", true);
...
void findReplace(const char* findStr, int findStrLen, const char* replStr, int replStrLen)
{
    ...
    {
        findBeeped = TRUE;
        throw_error("can't find '%s'", findStr);
    }
}
            ...
            else
            {
                ...
                switch(ch)
                {
                    ...
                    case 'O':       // copy tag-to-cursor into clipboard
                    {
                        ...
                        if (!clipBoard)
                            throw_error("no space for clipboard");
...
{
    if (macroLevel++ > 9)
        throw_error("macro recursion more than 9 deep");
            ...
            if (isFile)
            {
                ...
                *bcursPos = 0;
                ...
                else
                {
                    ...
                    if (!buf) throw_error("malloc failed");
...
int main(int argc, const char** argv)
{
    ...
    if ((exception_type = setjmp(exception_env)) == NO_EXCEPTION)
    {
        ...
        if (home)
            snprintf(clipName, MAX_LINE, "%s/.clipboard", home);
        else
            snprintf(clipName, MAX_LINE, "/tmp/.clipboard");
        ...
        for (i = 1; i < argc; i++)
        {
            ...
            {
                jmp_buf inner_env;
                memcpy(inner_env, exception_env, sizeof(jmp_buf));
                ...
                memcpy(exception_env, inner_env, sizeof(jmp_buf));
            }
        }
    ...
    do
    {
                ...
                switch (cmd1)
                {
                    case 'Q':
                        switch (cmd2)
                        {
                            case 'F':
                                initCmdBuf();
                                getCommand("find:", false);
                                snprintf(lastFind, MAX_LINE, "\4%s\4", bstart);
                                getCommand(currOptions(), false);
                            ...
                            case 'G':
                                initCmdBuf();
                                getCommand("go to line:", false);
                                insert(bstart, "QG\4", (long)3);
                                insert(beot, "\4", 1);
                                break;
                            case 'T':
                                initCmdBuf();
                                getCommand("tab size:", false);
                                insert(bstart, "QT\4", (long)3);
                                insert(beot, "\4", 1);
                                break;
                            case ' ':
                                initCmdBuf();
                                getCommand("enter command: (^L repeats)", false);
                                strcpy(lastFind, bstart);
                                break;
                            default:
                                doCommand(cmd2);
        ...
        else if (exception_type == EX_CANCEL)
        {
            cmdState = 0;
            selectBuffer(prevBuff);
            updateWindows();
